...
### 命名空间
1. 引入
一个学校在校园喇叭中广播：张三同学请尽快到校长办公室 请问，如果学校有多个学生名字都叫做张三，会怎样？
可想而知，全校所有名字是张三的同学都会离开跑到校长办公室。但是校长只需要1个张三，那么多张三都去是为什么呢？

答：就是因为名字乱导致的

试想，如果广播的内容是：一年级三班的张三同学立刻到校长办公室；此时就不会再出现上述那种情况了

同理
在C++编程中，为了避免张三、李四等多名开发人员编写了相同名字的变量或者函数，发明了一个区域限定的东西
...
大白话：就是用一对大括号将之前的变量、函数包裹起来，给这对大括号起个名字，想要用这些被包裹起来的变量、函数，就要先说明是哪个大括号中的
...
5. 示例
下面是一个使用命名空间的简单示例，展示了命名空间的定义、使用和嵌套：
...
int main() {
    ...
    // 使用嵌套命名空间中的函数
    std::cout << science::physics::calculateForce(10.0, 5.0) << std::endl;  // 输出: 50.0

    return 0;
}
```

在上述示例中，我们定义了两个命名空间 math 和 science::physics。math 命名空间包含一个 add 函数，用于执行整数相加。science::physics 命名空间嵌套在 science命名空间内部，其中定义了一个 calculateForce 函数，用于计算力的大小。

在 main 函数中，我们通过使用命名空间限定符来调用 math::add 和 science::physics::calculateForce 函数。我们演示了如何在不同的命名空间中定义和使用函数，并通过嵌套命名空间来组织相关的功能。
...
public:
    ...
    Student(int age) : age(age) { // 这个构造函数的作用是在创建一个Student对象时初始化对象的age成员变量。
...
在构造函数的初始化列表中，使用了冒号:，然后是age(age)，这表示将传递给构造函数的age参数的值赋给类的成员变量age，以完成对象的初始化。 例如，如果你创建一个Student对象并传递一个整数值作为参数，如下所示：

```c++
Student student1(20);
```

这将创建一个名为student1的Student对象，并将其年龄初始化为20，因为构造函数的初始化列表将传递的整数值20赋给了age成员变量。这样，student1对象的age成员变量的值就是20。
